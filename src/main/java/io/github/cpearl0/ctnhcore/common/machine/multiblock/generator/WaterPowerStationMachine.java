...
public class WaterPowerStationMachine extends CoilWorkableElectricMultiblockMachine {
    ...
    public WaterPowerStationMachine(IMachineBlockEntity holder) {
        super(holder);
        efficiency = 1 + 0.1 * getTraitOrThrow(CoilMachineTrait.class).getCoilTier();
    }
                ...
                for (int zoffset = -length + 1; zoffset < length; zoffset++) {
                    for (int yoffset = 0; yoffset > -4; yoffset--) {
                        if (Math.sqrt(Math.pow(xoffset, 2) + Math.pow(zoffset, 2)) <= length) {
                            if (level.getBlockState(pos.offset(xoffset, yoffset, zoffset))
                                    .equals(Blocks.WATER.defaultBlockState())) {
                                water++;
                            }
                        }
                    }
    ...
    public void addDisplayText(List<Component> textList) {
        if (isFormed()) {
            ...
            textList.add(Component.translatable("multiblock.ctnh.water_power_station2",
